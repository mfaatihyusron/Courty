...
class praktek extends CI_Controller {
    ...
    // Fungsi untuk menambah sport (oleh Role 1)
    public function add_sport()
    {
        // GANTI AKSES: Hanya untuk Role 1
        if (!$this->session->userdata('logged_in') || $this->session->userdata('role') != 1) {
            $this->session->set_flashdata('error', 'Anda tidak memiliki hak akses.');
            redirect('praktek/index');
            return;
        ...
        else
        {
            ...
            redirect('praktek/admin_dashboard'); // REDIRECT KE DASHBOARD ROLE 1
        }
    }
    
    // Fungsi untuk mengedit sport (oleh Role 1)
    public function edit_sport($id_sport)
    {
        ...
        if (!$sport) {
            ...
            redirect('praktek/admin_dashboard');
            return;
        }
        
        // Aturan validasi: Abaikan pengecekan is_unique jika nama tidak berubah
    ...
    // Fungsi untuk menghapus sport (oleh Role 1)
